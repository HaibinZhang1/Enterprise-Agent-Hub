...
| # | 一级入口 | 角色可见性 | 说明 |
|---|----------|-----------|------|
| 1 | 社区 | 全部 | 社区首页、全局搜索、Skill 发现与榜单入口；MCP/插件仅占位 |
...
│                    当前一级入口舞台区                       │
│  - 主页：单舞台 Agent 探索                                  │
│  - 社区：独立首页 Hero 搜索 + 三榜单 + 探索入口；结果页再进入左侧导航工作台 │
...
- **社区**：一级入口始终可见；已登录用户点击后进入独立社区首页，未登录用户点击时先弹出登录模态，登录成功后再进入社区首页。首页不显示左侧导航；点击探索或入口卡片后进入带左侧导航的社区工作台，工作台保留 `首页 / Skills / MCP / 插件`，并在作者分区提供 `发布 / 我的`
...
flowchart LR
    COMMUNITY_HOME["社区首页"]
    COMMUNITY_HOME -->|"搜索 / 探索"| COMMUNITY["社区 > Skills"]
    COMMUNITY_HOME -->|"技能 / MCP / 插件入口"| ENTRY["社区内部对应入口"]
    COMMUNITY_HOME -->|"发布"| PUBLISH["社区 > 发布"]
