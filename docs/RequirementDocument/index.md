...
- 文档版本：V1.6
- 当前状态：Desktop/API/Electron 主链、作者治理闭环、审核/管理入口、手机号登录、记住密码/自动登录、固定中文分类/标签、本地扫描能力、插件集中管理 P0、客户端在线升级需求已整合；Windows 安装包与 Linux live 部署仍待目标环境验收
- 适用范围：企业内网部署场景，Desktop/Electron 桌面客户端为主，Windows 安装包交付优先，服务端部署到 Linux 服务器
- 说明：本文档聚焦前端页面、功能、交互、权限、状态、业务规则、Skill 包规范与插件集中管理 P0 边界；不展开技术栈、数据库与接口实现细节。

> 当前交付边界（2026-04-28）：正式桌面交付仍以 Windows exe 为主，服务端部署仍以 Linux 服务器为目标并优先选择简单、兼容性高的方案。桌面运行时目标已调整为 Electron，以规避 Windows 系统 WebView 依赖导致的安装失败；真实 API `download-ticket` -> Electron 手动更新适配器 -> Central Store + SQLite -> 启用 -> 重启恢复的最小闭环必须保持不变。插件集中管理 P0 以 Extension 统一库存、审计/预检展示和 file-backed 写入门禁为交付边界；MCP Server、Hook、原生 Plugin、Agent CLI 在 P0 不做真实配置写入。剩余缺口集中在 Electron Windows exe 打包、Linux live 部署、客户端在线升级链路实现，以及 Windows 实机安装/更新验收。
...
- 客户端交付：正式安装包仍以 Windows exe 为主；Desktop/Electron 本地能力需承接现有 React UI、共享 API 契约与 P1 本地行为，并避免依赖系统 WebView；暂不提供 macOS、Linux 安装包或其他分发形式。
