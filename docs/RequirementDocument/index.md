...
> 当前发布边界（2026-04-09）：Windows-first 内网生产交付以 `apps/desktop` 为唯一维护的产品/演示入口。本需求集中的发布/审核、Web/管理台等内容代表完整产品蓝图或历史阶段上下文，本轮 Desktop 发布仅要求登录、连接状态、我的 Skill、市场/搜索/浏览、通知/状态；发布/审核 UX 延后到后续版本。
