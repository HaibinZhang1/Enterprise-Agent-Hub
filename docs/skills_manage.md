...
5. **安装方式可选**
   支持 `symlink`、`copy` 两种安装方式。
6. **扫描与去重**
   能扫描已存在的各工具目录，并基于真实路径去重，识别“同一个 skill 被多个工具引用”的情况。
7. **可扩展**
...
如果多个工具目录都只是软链到 `~/.ai-skills/skills/code-review`，扫描时应识别为同一个 skill，而不是多个副本。
...
- `scanner`: 路径扫描与识别
- `sync`: 同步决策
- `installer`: copy/symlink 执行
- `resolver`: 路径解析、realpath、变量展开
- `checksum`: 文件摘要与变更检测
...
**用中心库存真源，用 tool adapter 适配差异，用 path mapping 解决不同路径，用 symlink/copy 分发，用 realpath 去重。**