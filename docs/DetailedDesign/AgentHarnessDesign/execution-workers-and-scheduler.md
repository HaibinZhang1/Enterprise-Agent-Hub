...
- `docs/DetailedDesign/AgentHarnessDesign/shared-contracts-and-event-spine.md`：确认 Tool Action、Worker Session、Scheduler Trigger Outcome、EventEnvelope 与信任标记的共享语义。
- `docs/DetailedDesign/AgentHarnessDesign/data-classification-retention-matrix.md`：确认浏览器截图、文件清单、下载/上传、Worker Observation、日志与审计事件的敏感级、可见性和保留语义。
...
- 策略变为 Ask 时 Scheduler 创建 `awaiting_approval` Run，通知用户或管理员。
- 策略 Deny 时默认只写 `skipped_by_policy` Scheduler Event；合规策略要求时可创建 synthetic skipped Run。
...
以上 Execution Event 必须映射到 `shared-contracts-and-event-spine.md` 的 `EventEnvelope`。执行层可以保留内部事件名，但进入 Run Journal、Desktop Timeline 或 Admin Audit Query 前必须提供统一 `eventType`、`correlationId`、`policySnapshotRef`、`sensitivity`、`visibility`、`retention` 与 `auditProjection`。
...
2. Trust-marking pipeline 采用统一分工：Worker 标记来源与原始目标；Tool Gateway 归一化 trust / sensitivity / target；Context Builder 执行 prompt 边界与 token 裁剪；Result Composer 对高风险建议和未知引用做校验；Audit 记录 flags。
...
- 不把完整敏感网页、文件内容、截图或凭证明文无差别写入日志；默认按 `data-classification-retention-matrix.md` 保存摘要、hash、引用和脱敏状态。
...
4. Scheduler 错过补偿的概念默认值已统一为 skip / backfill_latest / merge / ask_user；后续实现需为首批任务类型配置默认策略和用户提示文案。
