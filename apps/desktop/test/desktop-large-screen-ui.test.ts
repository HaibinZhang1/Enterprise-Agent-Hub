...
test("community and local browsing opt into denser card and row container queries", () => {
  ...
  expectBlock(/@container local-pane \(min-width:\s*760px\)\s*\{[\s\S]*\.local-item\s*\{[\s\S]*grid-template-columns:\s*52px\s+minmax\(0,\s*1fr\)\s+auto;[\s\S]*padding:\s*12px\s+14px;[\s\S]*\.unified-skill-row\s*\{[\s\S]*min-height:\s*112px;/, "local browse rows should get denser card proportions");
});
