...
const DEFAULT_TRANSFORMS: Record<typeof DEFAULT_TOOL_IDS[number], string> = {
  codex: "codex_skill",
  claude: "claude_skill",
  cursor: "cursor_rule",
...
function makeDefaultTools(dataRoot: string, now: string): ToolConfig[] {
  return DEFAULT_TOOL_IDS.map((toolID) => ({
    ...
    transformStrategy: defaultTransformFor(toolID),
    lastScannedAt: now
  }));
}

function defaultTransformFor(toolID: string): string {
  return DEFAULT_TRANSFORMS[toolID as keyof typeof DEFAULT_TRANSFORMS] ?? "generic_directory";
}
...
export class ElectronLocalRuntime {
  ...
  async initialize(): Promise<void> {
    ...
    const migrationOptions: UserDataMigrationOptions = {
      ...
      ...(this.#options.productDataDirName === undefined ? {} : { productDataDirName: this.#options.productDataDirName }),
